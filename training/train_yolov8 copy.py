...
def run_training(
    *,
    ...
    save_dir = args.output_root.resolve() / run_name
    ...
    write_json(save_dir / "run_manifest.json", run_manifest)
    ...
    write_json(save_dir / "run_manifest.json", run_manifest)
    print(f"results_csv: {results_csv if results_csv.exists() else 'missing'}")
    print(f"best_pt: {best_pt if best_pt.exists() else 'missing'}")
    print(f"last_pt: {last_pt if last_pt.exists() else 'missing'}")

    if args.eval_test_after_train:
        print(f"Running test evaluation for {dataset_tag}...")
        model.val(data=str(data_yaml), split="test")
        run_manifest["status"] = "trained_and_tested"
        write_json(save_dir / "run_manifest.json", run_manifest)
